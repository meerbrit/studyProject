function [LP] = learningPhase(LP, numLearnTrials, win, pa_handle)
    ...
    for k=1:numLearnTrials %specify how many rows to be read from the structure, i.e. how big is the first block?
        ...
        %A1
        PsychPortAudio('FillBuffer', pa_handle, eval(LP(k).syll_1_1)); % loads data into buffer, first syllable from the LP structure above 
        ...
        %A2
        PsychPortAudio('FillBuffer', pa_handle, eval(LP(k).syll_1_2)); %second syllable
        LP(k).s12_start=toc;
        ...
        %X1
        PsychPortAudio('FillBuffer', pa_handle, eval(LP(k).syll_2_1)); %third syllable
        LP(k).s21_start=toc;
        ...
        PsychPortAudio('Stop', pa_handle, 1);
        %X2
        PsychPortAudio('FillBuffer', pa_handle, eval(LP(k).syll_2_2)); %fourth syllable
        LP(k).s22_start=toc;
        ...
        PsychPortAudio('Stop', pa_handle, 1);
        %B1
        PsychPortAudio('FillBuffer', pa_handle, eval(LP(k).syll_3_1)); %fifth syllable
        LP(k).s31_start=toc;
        ...
        PsychPortAudio('Stop', pa_handle, 1);
        %B2
        PsychPortAudio('FillBuffer', pa_handle, eval(LP(k).syll_3_2)); %sixth syllable
        LP(k).s32_start=toc;
